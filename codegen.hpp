...
public:
    ...
    /// To creat an expressions we need the last basic block calculated value
    ...
    CodeGenBlock(llvm::BasicBlock *blk,
            ...
            block(blk),
            // Rashed Al-Lahaseh - 00821573
            /// Add the retVal variable value for the declaiered 'returnValue' variable
            returnValue(retVal), locals(lc) {
    }
...
public:
    ...
    CodeGenContext() :
            MyContext(), irBuilder(MyContext), module("main", MyContext) {
              ...
              llvmTypes = {
                { "int", llvm::Type::getInt32Ty(this->MyContext) }, /// Already defined, just added for types
                { "void", llvm::Type::getVoidTy(this->MyContext) } /// Added void declaration
              };
    }
    ...
    // Rashed Al-Lahaseh - 00821573
    // Replace getIntType with getTypeOf functionality
